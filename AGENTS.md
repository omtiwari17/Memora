...
- **Strict Category Uniqueness & Seeding Integrity** -
  - `Category` model level (`quotes/models.py`): `clean()` and `save()` methods enforce that duplicate category names (`name__iexact`) or slugs cannot be saved to the database.
  - `category_create`: Validates case-insensitively against all system defaults and user categories (`Q(name__iexact=name) | Q(slug=slug)`). Rejects duplicates with a clear message: `"Duplicate detected: A category named '<name>' already exists. Category names must be unique."`
  - `category_edit`: Enforces the same uniqueness check across all existing categories, excluding the current category ID.
  - `category_manage.html`: Real-time collision detection on both Create and all Inline Edit forms. If a duplicate is typed, highlights red with glowing ring, displays an immediate warning box, disables submit buttons, blocks `onsubmit` (even on physical Enter key press) with shake animation, and injects a prominent glowing `Duplicate Not Saved` error banner.
  - Protected System Categories: Core default categories cannot be deleted by users, displaying a `System Default` lock badge.
...
- [x] **Slim, Calm Sidebar (`w-60`)**:
  ...
  - Streamlined bottom dock with creator links (`omtiwari.dev`, GitHub, LinkedIn) and clean user vault profile.
